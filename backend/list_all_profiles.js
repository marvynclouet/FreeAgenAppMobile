...
async function listAllProfiles() {
  ...
  try {
    ...
    const [teams] = await connection.execute(`
      SELECT id, name, city, description, level, division, founded_year
      FROM teams 
      ORDER BY created_at DESC
    `);
    ...
    const [players] = await connection.execute(`
      SELECT pp.id, u.name, u.email, pp.age, pp.position, pp.height, pp.weight, pp.level
    ...
    const [handibasket] = await connection.execute(`
      SELECT hp.id, u.name, u.email, hp.age, hp.position, hp.handicap_type, hp.level
    ...
    const [coaches] = await connection.execute(`
      SELECT cp.id, u.name, u.email, cp.experience_years, cp.level, cp.specialization
    ...
    const [clubs] = await connection.execute(`
      SELECT cp.id, u.name, u.email, cp.club_name, cp.city, cp.level, cp.division
